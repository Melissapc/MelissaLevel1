...
public class TurtlePond implements KeyEventDispatcher {
	...
	private void checkForFood() throws Exception {
		...
		if (Math.abs(cookieX - tortoiseLocationX) < 100
				&& (Math.abs(cookieY - tortoiseLocationY) < 100)) {
		...
		if (Math.abs(cookieY - tortoiseLocationY) < 50
				&& (Math.abs(cookieX - tortoiseLocationX) < 50)) {
			Tortoise.getBackgroundWindow().setBackground(Color.ORANGE);
		}
		// 9. If the Tortoise is within 20 pixels of the food, set the
		// background color to red
		if (Math.abs(cookieX - tortoiseLocationX) < 20
				&& (Math.abs(cookieY - tortoiseLocationY) < 20)) {
		...
		if (Math.abs(cookieX - tortoiseLocationX) < 5
				&& (Math.abs(cookieY - tortoiseLocationY) < 5)) {
